...
void Console::Ws2812b::Pwm::BitStream::push_back(bool bit,uint32_t nticks)
{
  for (auto i=0u ; i<nticks ; ++i)
    this->push_back(bit) ;
}

void Console::Ws2812b::Pwm::BitStream::push_back(Timing::Nticks const &t,uint32_t rgb)
{
...
void Console::Ws2812b::Pwm::BitStream::push_back(Timing::Nticks const &t,uint32_t rgb,size_t n)
{
  for (unsigned i=0 ; i<n ; ++i)
    this->push_back(t,rgb) ;
}

std::vector<uint32_t> Console::Ws2812b::Pwm::BitStream::to_uint32() const
{
...
void Console::Ws2812b::Pwm::start()
{
...
void Console::Ws2812b::Pwm::send(uint32_t nleds,uint32_t rgb)
{
  BitStream q ;
  q.push_back(0,t.res) ;
...
void Console::Ws2812b::Pwm::fill_fifo(uint32_t const*p,unsigned n)
{
...
void Console::Ws2812b::Pwm::send_fifo(uint32_t const *rgb,unsigned n)
{
  for (decltype(n) i=0 ; i<n ; ++i) {
