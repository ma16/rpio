...
static void blck_0n(rep_t rep,size_t nwords,uint32_t *d,Ui::ArgL *argL)
{
  ...
  if (m < 1 || MAX < m)
    throw Neat::Error("Console:Throughput:invoke:" + std::to_string(m) + " not in range") ;
  argL->finalize() ;
...
{
  argL->finalize() ;
  Neat::make_safe(nwords) * sizeof(uint32_t) ;
  auto t0 = now() ;
  for (decltype(rep) i=0 ; i<rep ; ++i)
...
static void invoke_0n(Rpi::Peripheral *rpi,rep_t rep,Ui::ArgL *argL)
{
  ...
  if (nwords < 2)
    throw Neat::Error("Console:Throughput:invoke:at least two words required") ;
